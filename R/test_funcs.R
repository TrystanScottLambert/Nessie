...
.bijcheck <- function(grefs1, grefs2, groupcut = 2) {
  ...
  for (i in seq_along(grouplist)) {
    ...
    if (is.na(temptab[bestloc, 1]) == FALSE) {
      bijout <- rbind(bijout, c(
        grouplist[i], temptab[bestloc, 1], i, matchrow,
        tab1[tab1[, 1] == grouplist[i], 2], tab2[tab2[, 1] == temptab[bestloc, 1], 2],
        frag, frac1[bestloc], frac2[bestloc], comb[bestloc]
      ))
    } else {
      bijout <- rbind(bijout, c(
        grouplist[i], NA, i, NA, tab1[tab1[, 1] == grouplist[i], 2], 1, frag,
  ...
  G1eff <- G1bij * G1int
  return(list(groups = bijout, summary = c(
    bij_num = G1bij_num, bij_den = G1bij_den, bij = G1bij, per = G1perfect,
